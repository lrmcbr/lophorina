...
def demonstracao(giro_helices, t, alvo):
    ...
    giros = [giro_helices[0], -giro_helices[1], -giro_helices[2], giro_helices[3]]

    output(['Planando a ' + str(alvo) + ' m'], alvo - robot_pose[4][2],0)
...
def FlightController():
    ...
    global controller0
    controller0 = PID(Kp, Ki, Kd)
    controller0.send(None)
    ...
    while not rospy.is_shutdown() and not roboVirado() and not fim:
        ...
        giro_helices[1] = controller1.send([t, robot_pose[1][2], alvo])
        giro_helices[2] = controller1.send([t, robot_pose[2][2], alvo])
        giro_helices[3] = controller1.send([t, robot_pose[3][2], alvo])
